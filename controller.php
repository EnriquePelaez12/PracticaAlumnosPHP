...
switch($_POST['opc'])
{
 ...
 case "guardar":    
 try{
          $sql = $conexion->prepare("INSERT INTO alumno(nombre_alum,apellido_alum,edad_alum,record)
          VALUES('".$_POST['nombre']."','".$_POST['apellido']."','".$_POST['edad']."','".$_POST['record']."')");       
          $sql->execute();         
...
case "eliminar":
try{
         $sql = $conexion->prepare("DELETE FROM alumno WHERE id_alumno =".$_POST['clave']);       
         $sql->execute();         
         //header("location:index.php");   
   }
     catch (PDOException $e) {
